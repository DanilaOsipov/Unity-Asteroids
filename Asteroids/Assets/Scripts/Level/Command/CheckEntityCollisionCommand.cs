...
namespace Level.Command
{
    public class CheckEntityCollisionCommand : ICommand
    {
        ...
        public void Execute()
        {
            ...
            var enemyCheck = (_type == EntityType.Enemy || _type == EntityType.SmallAsteroid 
                            || _type == EntityType.BigAsteroid) && damagable.IsFriendlyToPlayer;
            var playerCheck = _type == EntityType.Player && !damagable.IsFriendlyToPlayer;
            if (!enemyCheck && !playerCheck) return;
